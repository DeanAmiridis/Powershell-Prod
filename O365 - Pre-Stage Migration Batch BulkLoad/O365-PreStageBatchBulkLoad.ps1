...
foreach ( $Mailbox in $Mailboxes ) {
    New-MoveRequest -Identity $Mailboxes.Email -remote -RemoteHostName ($HybridURL) -TargetDeliveryDomain $TargetDeliveryDomain -RemoteCredential $UserCredential -AcceptLargeDataLoss -BadItemLimit 50 -LargeItemLimit 10 -BatchName $Mailboxes.Name
