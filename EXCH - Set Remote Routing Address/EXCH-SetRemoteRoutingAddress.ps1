...
$RM_Accounts = Import-Csv -Path '.\RM_ImportUsersConsolidated.csv'
...
foreach ( $RM_Account in $RM_Accounts ) {
    $Name = $RM_Account.Name
    $samAccountName = $RM_Account.SAMAccountName
    Set-RemoteMailbox -Identity "$Name" -RemoteRoutingAddress "$($SAMAccountName)$RoutingAddressSuffix"
}  